...
//!
//! `pub mod data;`, `pub mod eda;`, `pub mod features;`, and `pub mod
//! split;` below do two things at once:
//! 1. Tell Rust to compile `src/data.rs`, `src/eda.rs`, `src/features.rs`,
//!    and `src/split.rs` as submodules of this crate (the filename becomes
//!    the module name).
//! 2. Mark them `pub` (public) so code outside this crate — like the
//!    `charts` and `models` crates, or this crate's own `src/bin/`
//!    binaries — can call `core::data::...`, `core::eda::...`,
//!    `core::features::...`, and `core::split::...`.
//!
